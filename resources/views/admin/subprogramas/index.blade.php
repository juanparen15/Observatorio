                        ...
                        <table id="example2" class="table table-hover text-nowrap">
                            <thead>
                                <tr>
                                    ...
                                    <th>CODIGO DE SUBPROGRAMA</th>
                                    <th>NOMBRE DE SUBPROGRAMA</th>
                                    <th>ACCIONES</th>
                                </tr>
                            <tbody>
                                </thead>

                                @foreach ($subprograma as $subprograma)
                                    <tr>
                                        <td>{{ $subprograma->id }}</td>
                                        <td>{{ $subprograma->programa->nomProg }}</td>
                                        <td>{{ $subprograma->codSP }}</td>
                                        <td>{{ $subprograma->nomSP }}</td>

                                        <td class="project-actions text-right">
                                            <form action="{{ route('admin.subprogramas.destroy', $subprograma) }}"
                                                method="POST">
                                                @csrf
                                                @method('delete')

                                                <a class="btn btn-primary btn-sm"
                                                    href="{{ route('admin.subprogramas.edit', $subprograma) }}">Editar</a>
